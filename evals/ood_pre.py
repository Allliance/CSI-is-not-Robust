...
class DifferentiableScoreModel(nn.Module):
  ...
  def get_scores(self, feats_dict, x):
    ...
    for f_sim, f_shi in zip(feats_sim, feats_shi):
        ...
        for shi in range(P.K_shift):
            score = score + (f_sim[shi] * P.axis[shi].to(device)).sum(dim=1).requires_grad_().max() * torch.tensor(P.weight_sim[shi], requires_grad=True).to(device)
  ...
  def get_features(self, data_name, model, data_batch, interp=False,
    ...
    if len(left) > 0:
        _feats_dict = self._get_features(model, data_batch, interp,
  ...
  def _get_features(self, model, data_batch, interp=False, simclr_aug=None,
...
def eval_ood_detection(P, model, id_loader, ood_loaders, ood_scores, train_loader=None, simclr_aug=None):
    ...
    P.attack = {'FGSM': FGSM(score_model, eps=P.eps),
                'PGD': PGD(score_model, steps=P.steps, eps=P.eps, alpha=P.alpha)
    }[P.desired_attack]
    
    if P.print_score:
        print('Pre-compute features...')
    feats_id = get_features(P, P.dataset, model, id_loader, prefix=prefix, attack=P.in_attack, is_ood=False, **kwargs)  # (N, T, d)
    feats_ood = dict()
    for ood, ood_loader in ood_loaders.items():
        if ood == 'interp':
            feats_ood[ood] = get_features(P, ood, model, id_loader, interp=True, prefix=prefix, attack=P.out_attack,is_ood=True, **kwargs)
        else:
            feats_ood[ood] = get_features(P, ood, model, ood_loader, prefix=prefix, attack=P.out_attack, is_ood=True, **kwargs)
